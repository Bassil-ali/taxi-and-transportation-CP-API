...
class CategoryRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if (in_array(request()->method(), ['PUT', 'PATCH'])) {
            ...
            $rules['name.*'] = ['required','string','min:2','max:50', UniqueTranslationRule::for('categories', 'name')->ignore($city?->id)];

        } else {

            $rules['name.*'] = ['required','string','min:2','max:50', UniqueTranslationRule::for('categories', 'name')];
