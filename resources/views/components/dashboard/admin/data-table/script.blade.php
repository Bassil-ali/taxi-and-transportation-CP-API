<script>
    ...
    $.map(header, (value, index) => ExColumns.push((index + 1)));

    function ExColumnsFun(newExColumns, isSelected) 
    {
        sum = isSelected.length != ExColumns.length;
        console.log(sum ? isSelected : ExColumns);
        return sum ? isSelected : ExColumns;
    ...
    myColumns.push({data: 'created_at', name: 'created_at', searchable: false, width: '14%'});
    ...
    let Table = $('#data-table').DataTable({
        ...
        serverSide: true,
        processing: true,
        language: {url: "{{ asset('assets/plugins/custom/datatables/lang/' . $lang . '.json') }}"},
    ...
    $(".export-items").on('click', function(e) {
        ...
        Table.button('.buttons-' + $(this).data('export')).trigger();
    });

    $('#columns').on('change', function (e) {
        
        var notSelected = $.map($('select#columns option:not(:selected)'), (option) => {return option.value});
        var isSelected  = $.map($('select#columns option:selected'), 
                                (option) => {
                                    if (option.text == 'record_select' || option.text == 'actions') {} else {return option.value}
                                }
                            );

        Table.columns(notSelected).visible(false);
        Table.columns($('#columns').val()).visible(true);
        
        ExColumnsFun(ExColumns, isSelected);
