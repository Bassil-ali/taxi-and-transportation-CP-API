<x-dashboard.admin.layouts.app>
    ...
    <x-slot name="title">{{ trans('admin.menu.categories') }}</x-slot>

    <div class="card shadow-sm">

        <div class="card-header">
            <h3 class="card-title">{{ trans('admin.menu.categories') }}</h3>
        </div>
            ...
            <div class="card-toolbar">
                @if(permissionAdmin('create-categories'))
                    <div class="m-2">
                        ...
                        <x-dashboard.admin.btn.add route="dashboard.admin.app.categories.create"/>
                    </div>
                @endif

                @if(permissionAdmin('delete-categories'))
                    <x-dashboard.admin.data-table.btn.bulk-delete route="dashboard.admin.app.categories.bulk_delete"/>
                @endif
            </div>
